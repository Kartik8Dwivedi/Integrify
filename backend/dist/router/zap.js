...
router.post("/", middleware_1.authMiddleware, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Validate each action's availableActionId
    for (const action of parsedData.data.actions) {
        const actionExists = yield db_1.prismaClient.availableAction.findUnique({
            where: { id: action.availableActionId },
        });
        if (!actionExists) {
            return res
                .status(400)
                .json({ message: `Invalid actionId: ${action.availableActionId}` });
        }
    }
    // Validate triggerId
    const triggerExists = yield db_1.prismaClient.availableTrigger.findUnique({
    ...
    if (!triggerExists) {
        return res
            .status(400)
            .json({
            message: `Invalid triggerId: ${parsedData.data.availableTriggerId}`,
        });
    }
